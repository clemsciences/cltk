...
def generate_pos(
    ...
    if not doc.backend:
        msg_no_backend: str = "Doc must have `.backend` set to 'openai', 'mistral', 'anthropic', 'ollama', or 'ollama-cloud' to use generate_pos."
    ...
    if doc.backend == "openai":
        ...
        raise CLTKException(
            f"Unsupported backend for generate_pos: {doc.backend}. Use 'openai', 'mistral', 'anthropic', 'ollama', or 'ollama-cloud'."
        )
...
def generate_gpt_morphosyntax(
    ...
    *,
    ...
    if doc.backend == "openai":
        ...
        raise CLTKException(
            f"Unsupported backend for morphosyntax: {doc.backend}. Use 'openai', 'mistral', 'anthropic', 'ollama', or 'ollama-cloud'."
        )
    if not doc.normalized_text:

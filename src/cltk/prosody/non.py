...
import cltk.phonology.orthophonology as old_norse_orthophonology
from cltk.alphabet.non import old_norse_normalize
...
class OldNorseProsodyAnalysis(ProsodyAnalysis):
    def __init__(self):
        self.alliterations = []

    def set_alliterations(self, alliterations):
        self.alliterations = alliterations


class OldNorseMeter(enum.Enum):
    FORNYRDHISLAG = enum.auto()
    LJOODHHAATR = enum.auto()


class OldNorseProsody:
    def __init__(self):
        self.verse_separators = ["\n"]

    def scan(self, verse: List[Word]) -> List[Tuple[Word, Word]]:
        # result = OldNorseProsodyAnalysis()
        alliterations = []
        for j, word in enumerate(verse):
            if j < len(verse) - 1:
                for k, word_k in enumerate(verse[j+1:]):
                    if not word.stop and not word_k.stop:
                        if isinstance(word.phonetic_object_transcription[0], old_norse_orthophonology.Consonant) and \
                                word.phonetic_object_transcription[0].ipa == word_k.phonetic_object_transcription[0].ipa:
                            alliterations.append((word, word_k))
                        elif isinstance(
                                word.phonetic_object_transcription[0], old_norse_orthophonology.Vowel
                        ) and isinstance(word_k.phonetic_object_transcription[0], old_norse_orthophonology.Vowel):
                            alliterations.append((word, word_k))
    ...
    #
    # def split_paragpraphs(self, old_norse_meter: OldNorseMeter):
    #     if old_norse_meter == OldNorseMeter.FORNYRDHISLAG:
    #         pass
    #     elif old_norse_meter == OldNorseMeter.LJOODHHAATR:
    #         pass

    def __repr__(self):
        return f"<OldNorseProsody>"
